...
export class EmployeeAssignmentRepository {
    ...
    private async triggerEvent(data: EmployeeAssignmentEntityEvent | EmployeeAssignmentUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-jobs-JobAssignment-EmployeeAssignment", ["trigger"]);
        ...
        producer.topic("codbex-jobs-JobAssignment-EmployeeAssignment").send(JSON.stringify(data));
    }
}
