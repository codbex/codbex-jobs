...
export interface JobRoleEntityEvent {
...
export interface JobRoleUpdateEntityEvent extends JobRoleEntityEvent {
...
export class JobRoleRepository {
    ...
    public findById(id: number, options: JobRoleEntityOptions = {}): JobRoleEntity | undefined {
    ...
    private async triggerEvent(data: JobRoleEntityEvent | JobRoleUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-jobs-Companies-JobRole", ["trigger"]);
        ...
        producer.topic("codbex-jobs-Companies-JobRole").send(JSON.stringify(data));
    }
}
